...
minetest.register_entity('bows:arrow_entity', {
	...
	on_activate = function(self, staticdata)
		...
		local def = bows.registered_arrows [self.arrow]
        local desc = def.description or self.arrow
		self.object:set_properties({
			textures = {'bows:arrow_node'},
			infotext = desc
		})
	end,
		...
		if not self._attached and not self._in_liquid then
			...
			if self._tflp >= self._tool_capabilities.full_punch_interval then
				if self._is_critical_hit then
					bows.particle_effect(self._old_pos, 'arrow_crit')
				else
					bows.particle_effect(self._old_pos, 'arrow')
				end
			end
		end
		...
		while pointed_thing do
			...
			elseif pointed_thing.type == 'node' and not self._attached then
				...
				if node_def.drawtype == 'liquid' and not self._is_drowning then
					...
					bows.particle_effect(self._old_pos, 'bubble')
				...
				if bows.mesecons and node.name == 'bows:target' then
