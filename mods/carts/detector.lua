...
function carts:turnoff_detector_rail(pos)
	...
	if core.get_item_group(node.name, "detector_rail") == 1 then
		if node.name == "carts:detectorrail_on" then --has not been dug
			core.swap_node(pos, {name = "carts:detectorrail", param2=node.param2})
		end
...
function carts:signal_detector_rail(pos)
	...
	if node.name == "carts:detectorrail" then
		core.swap_node(pos, {name = "carts:detectorrail_on", param2=node.param2})
	end
	mesecon.receptor_on(pos, mesecons_rules)
	core.after(0.5, carts.turnoff_detector_rail, carts, pos)
end
