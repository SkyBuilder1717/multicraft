...
local function legal_equip(index, itemname)
	if index == 1 and not string.find(itemname, "helmet") then
		return false
	elseif index == 2 and not string.find(itemname, "chestplate") then
		return false
	elseif index == 3 and not string.find(itemname, "leggings") then
		return false
	elseif index == 4 and not string.find(itemname, "boots") then
		return false
	end
	return true
end
...
local function init_player_armor(initplayer)
	...
	local armor_inv = minetest.create_detached_inventory(name.."_armor", {
		...
		allow_put = function(inv, listname, index, put_stack, player)
			...
			if not legal_equip(index, put_stack:get_name()) then
				return 0
			end
		...
		allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
			...
			if not legal_equip(to_index, stack:get_name()) then
				return 0
			end
			return count
		end,
