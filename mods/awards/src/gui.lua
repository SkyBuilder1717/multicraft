...
function awards.get_formspec(name, to, sid)
	...
	if #awards_list == 0 then
		formspec = formspec .. "label[3.9,1.5;"..core.formspec_escape(S("Error: No achivements available.")).."]"
	...
	if sdef and sdef.secret and not sitem.unlocked then
		formspec = formspec .. "label[1,2.75;"..
				core.formspec_escape(S("(Secret Award)")).."]"..
	...
	for _, award in pairs(awards_list) do
		...
		if def then
			...
			if def.secret and not award.unlocked then
				formspec = formspec .. "#707070"..core.formspec_escape(S("(Secret Award)"))
			else
...
function awards.show_to(name, to, sid, text)
	...
	if name == to and data.disabled then
		core.chat_send_player(name, S("You've disabled awards. Type /awards enable to reenable."))
		return
	end
	if text then
		local awards_list = awards.get_award_states(name)
		if #awards_list == 0 then
			core.chat_send_player(to, S("Error: No award available."))
			return
		elseif not data or not data.unlocked  then
			core.chat_send_player(to, S("You have not unlocked any awards."))
			return
		end
		core.chat_send_player(to, string.format(S("%s’s awards:"), name))
		...
		core.show_formspec(to,"awards:awards",
			"size[8,8.6]background[0,0;8,8.6;formspec_empty.png;false]" .. deco ..
