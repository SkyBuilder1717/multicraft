...
-------------------------------------------------------------------------------
-- shuffling and positioning at random positions items
-------------------------------------------------------------------------------
function villages.shuffle(inv)
	local tbl = inv:get_list("main")
	for i, stack in pairs(tbl) do
		if stack:get_count() > 1 then
			local rndm = math.random(1, #tbl)
			if inv:get_stack("main", rndm):is_empty() then 
				inv:set_stack("main", rndm, stack:get_name().." 1")
			end
			stack:replace(stack:get_name().." "..(stack:get_count() - 1))
		end
	end
	inv:set_list("main", shuffle(inv:get_list("main")))
end
-------------------------------------------------------------------------------
-- fill chests
-------------------------------------------------------------------------------
