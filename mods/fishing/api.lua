function fishing.distance(pos1, pos2)
...
local function walkable(pos) return (core.get_node(pos).name ~= "air" and false) end

function fishing.visible(ob, pos2)
	...
	v.z = (v.z  / amount)*-1
	for i=1,d,1 do
		local node = core.registered_nodes[core.get_node({x=pos1.x+(v.x*i),y=pos1.y+(v.y*i),z=pos1.z+(v.z*i)}).name]
...
local function apos(pos, x, y, z) return {x = pos.x + (x or 0), y = pos.y + (y or 0), z = pos.z + (z or 0)} end
	...
	core.after(1, function()
		if self.activity then
			...
			if math.random(1, 8) == 8 then
				self.peck=true
				core.sound_play("fishing_string", {to_player = self.user_name})
				core.after(3, function()
					if self.activity then
						self:delete()
						core.sound_play("fishing_back", {to_player = self.user_name})
					end
				end)
				return
			end
...
core.register_on_leaveplayer(function(user)
	fishing.clear(user)
end)

core.register_on_dieplayer(function(user)
	fishing.clear(user)
end)

core.register_entity("fishing:fishing_string",{
	initial_properties = {
		physical = false,
		visual = "cube",
		pointable = false,
	...
	on_activate = function(self, staticdata)
		for _, ob in pairs(core.get_objects_inside_radius(self.object:get_pos(), 1)) do
...
core.register_entity("fishing:fishing_float",{
	initial_properties = {
		physical = false,
		decoration = false,
	...
	on_activate = function(self, staticdata)
		...
		self.string = core.add_entity(self.object:get_pos(), "fishing:fishing_string")
	...
	on_step = function(self,dtime)
		if not (self.user and self.string and self.user:get_wielded_item():get_name() == "fishing:fishing_rod") or fishing.distance(self.object,self.user) > fishing.blocks_far or not (fishing.visible(self.object,self.user)) then
			self:delete()
			return
		end
		...
		if fishing.distance(pos1,pos2) > fishing.blocks_move then
			...
			if fishing.distance(pos1,pos2) < 1 then
				...
				for _, ob in pairs(core.get_objects_inside_radius(pos3, 2)) do
					local en = ob:get_luaentity()
			...
			return
		elseif core.get_item_group(core.get_node(pos1).name,"water") > 0 then
			if self.peck then
...
core.register_tool("fishing:fishing_rod", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		for _, ob in pairs(core.get_objects_inside_radius(pos, 30)) do
			local en = ob:get_luaentity()
		...
		local f = core.add_entity(apos(pos,0,1.5), "fishing:fishing_float")
